...
asmlinkage long sys_wait4(pid_t pid,unsigned int * stat_addr, int options, struct rusage * ru)
{
	int flag, retval;
	//HW1
	if(current->feature_status == 1 && current->privilege_level < 1){
		current->log_array[current->num_of_error].syscall_req_level = 1;
		current->log_array[current->num_of_error].proc_level = current->privilege_level;
		current->log_array[current->num_of_error].time = jiffies;
		(current->num_of_error)++;
		retval = -EINVAL;
		return retval;
	}
	//HW1
